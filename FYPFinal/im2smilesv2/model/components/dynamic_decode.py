...
def transpose_batch_time(t):
    if t.shape.ndims == 2:
        return tf.transpose(a=t, perm=[1, 0])
    elif t.shape.ndims == 3:
        return tf.transpose(a=t, perm=[1, 0, 2])
    elif t.shape.ndims == 4:
        return tf.transpose(a=t, perm=[1, 0, 2, 3])
    else:
...
def dynamic_decode(decoder_cell, maximum_iterations):
    ...
    try:
        maximum_iterations = tf.convert_to_tensor(value=maximum_iterations,
                dtype=tf.int32)
    except ValueError:
        pass
    ...
    def condition(time, unused_outputs_ta, unused_state, unused_inputs,
        finished):
        return tf.logical_not(tf.reduce_all(input_tensor=finished))
