...
class Decoder(object):
    ...
    def __call__(self, training, img, formula, dropout):
        ...
        E = tf.compat.v1.get_variable("E", initializer=embedding_initializer(),
                shape=[self._n_tok, dim_embeddings], dtype=tf.float32)

        start_token = tf.compat.v1.get_variable("start_token", dtype=tf.float32,
                shape=[dim_embeddings], initializer=embedding_initializer())

        batch_size = tf.shape(input=img)[0]

        # training
        with tf.compat.v1.variable_scope("attn_cell", reuse=False):
            ...
            train_outputs, _ = tf.compat.v1.nn.dynamic_rnn(attn_cell, embeddings,
                    initial_state=attn_cell.initial_state())

        # decoding
        with tf.compat.v1.variable_scope("attn_cell", reuse=True):
...
def get_embeddings(formula, E, dim, start_token, batch_size):
    ...
    formula_ = tf.nn.embedding_lookup(params=E, ids=formula)
